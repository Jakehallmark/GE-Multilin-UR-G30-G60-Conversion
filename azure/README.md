# Azure Function ΓÇö G30 to G60 Converter
...
Used with **SharePoint ΓåÆ Power App ΓåÆ Power Automate** for browser-based conversion. The desktop `.bat` workflow in the repo root is unchanged.
...
azure/
  ...
  sync-from-root.ps1         # Copy converter + bases/ from repo root
  ...
  function_app/
    ...
    bases/                   # Synced G60 Base*.xml templates
    G60 Base.xml             # Default template copy (for deploy check)
    host.json
    requirements.txt
```
...
1. Portal ΓåÆ Function App ΓåÆ Configuration ΓåÆ General settings ΓåÆ **SCM Basic Auth** ON ΓåÆ **Apply** ΓåÆ **Restart**
...
When `convert_g30_to_g60.py` or any file under `bases/` changes at the repo root:
